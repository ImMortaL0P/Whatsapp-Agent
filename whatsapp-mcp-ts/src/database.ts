...
export function initializeDatabase(): DatabaseSync {
  ...
  // Dynamically check and add unread_count column if it doesn't exist
  try {
    const tableInfo = db.prepare("PRAGMA table_info(chats)").all() as any[];
    const hasUnreadCount = tableInfo.some((col) => col.name === "unread_count");
    if (!hasUnreadCount) {
      db.exec("ALTER TABLE chats ADD COLUMN unread_count INTEGER DEFAULT 0;");
    }
  } catch (err) {
    console.error("Failed to migrate database to add unread_count column:", err);
  }
...
export function storeChat(chat: Partial<Chat> & { jid: string }): void {
  ...
  try {
    const stmt = db.prepare(`
            INSERT INTO chats (jid, name, last_message_time, unread_count)
            VALUES (@jid, @name, @last_message_time, @unread_count)
      ...
      last_message_time:
        chat.last_message_time instanceof Date
          ...
          : chat.last_message_time === null
            ? null
            : chat.last_message_time !== undefined
              ? String(chat.last_message_time)
              : null,
      unread_count: chat.unread_count !== undefined ? chat.unread_count : null,
    });
  } catch (error) {
...
export function getUnreadMessagesGrouped(): UnreadGroup[] {
  const db = getDb();
  try {
    ...
    for (const chatRow of chatRows) {
      const jid = chatRow.jid;
      const unreadCount = Number(chatRow.unread_count);
      if (unreadCount <= 0) continue;

      // Get the contact/chat display name
      const contactStmt = db.prepare(`
        SELECT COALESCE(name, notify, phone_number) as display_name FROM contacts WHERE jid = ?
      `);
      const contactRow = contactStmt.get(jid) as any | undefined;
      const chatName = chatRow.name ?? contactRow?.display_name ?? jid.split("@")[0] ?? "Unknown Chat";

      // Get the last unreadCount messages where is_from_me = 0
      ...
      `);
      const messageRows = messagesStmt.all(jid, unreadCount) as any[];
    ...
    console.error("Error getting unread messages grouped:", error);
    return [];
  }
}

export function incrementUnreadCount(chatJid: string): void {
  const db = getDb();
  try {
    const stmt = db.prepare(`
      UPDATE chats

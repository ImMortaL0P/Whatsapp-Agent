...
export async function startMcpServer(
  ...
  server.tool(
    ...
    {},
    async () => {
      mcpLogger.info("[MCP Tool] Executing get_unread_messages");
      try {
        const unreadGroups = getUnreadMessagesGrouped();
        return {
          content: [
            {
              type: "text",
              text: JSON.stringify(unreadGroups, null, 2),
            },
          ],
        };
      } catch (error: any) {
        mcpLogger.error(
          `[MCP Tool Error] get_unread_messages failed: ${error.message}`
        );
        return {
          isError: true,
          content: [
            {
              type: "text",
              text: `Error getting unread messages: ${error.message}`,
            },
          ],
        };
      }

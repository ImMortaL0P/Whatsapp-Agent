...
import { startExpressServer } from "./server.ts";
...
async function main() {
  ...
  try {
    ...
    mcpLogger.info("Starting Express Server for Chief of Staff dashboard...");
    startExpressServer(3002);
    mcpLogger.info("Express Server started.");
  } catch (error: any) {
    mcpLogger.fatal(
      { err: error },
